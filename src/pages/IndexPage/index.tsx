...
function IndexPage() {
    ...
    const swiperParams: SwiperOptions = {
      ...
      on: {
        slideChangeTransitionStart: function (swiper) {
          console.log(document.getElementById(`${swiper.previousIndex}`)!.style.opacity);

          // document.getElementById(`${swiper.previousIndex}`)!.style.animationPlayState = "paused"
          MainParticle?.ChangeModel(modelList[swiper.activeIndex]);
          setPageIndex(swiper.activeIndex)
          // document.getElementById(`${swiper.activeIndex}`)!.style.animationPlayState = "running"
        },
      }
    };
  ...
  function nameInit() {
    try {
      ...
      alert('我劝你最好老实点')
      window.location.replace('https://www.turingteam.me:8082/')
    }
  ...
  return (
    <div className={Styles.index_page}>
      <div className={`${Styles.header}`}>
        <div className={`${Styles.screen} ${mobile ? Styles.hidden : ''}`}>
      ...
      <div className={Styles.container}>
        <div className={`swiper ${Styles.test}`}>
          <div className="swiper-wrapper">
            <div className={`swiper-slide ${mobile ? Styles.t1mobile : Styles.t1PC} ${pageIndex === 0 ? Styles.fadein : ''}`} id='0'>
              <p>你好，{name}</p>
              <p>野外捡几个土豆就能做，</p>
              <p>吃了加34%的血两个吃的饱饱的，</p>
              <p>缺的营养这一块儿也补上了。</p>
            </div>
            <div className={`swiper-slide ${mobile ? Styles.t2mobile : Styles.t2PC} ${pageIndex === 1 ? Styles.fadein : ''}`} id='1'>
              <p>我们将在11月13日14:30分，</p>
              <p>于科技楼（图海楼）423进行方向宣讲，</p>
              <p>届时将当场选择方向</p>
            </div>
            <div className={`swiper-slide ${Styles.t3} ${pageIndex === 2 ? Styles.fadein : ''}`} id='2'><p>进入下一轮的挑战</p></div>
          </div>
        </div>
      </div>
